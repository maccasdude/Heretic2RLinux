...
Per-version development history of the Heretic II Vulkan renderer (ref_vk),
oldest first (v46 -> v119). For current status, documented non-gaps, and the
regression-trap invariants, see TODO.md; this file is the historical record only.
Interleaved sections without a version number (architecture maps, audits, and
diagnostic notes) are kept in place as the context for the entries around them.
